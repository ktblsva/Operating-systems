...
void sendFile(char* filename, SOCKET my_sock)
{
    ...
    fclose(fp); 
	char* toSend = {'\0'};
	sprintf(toSend, "File: transfer %s successful!", filename);
	writeToLogfile(toSend);
}
...
void receiveFile(SOCKET sock)
{
    ...
    fclose(fp);
	char* toSend = {'\0'};
	sprintf(toSend, "Receive File: %s from client successful!", CONFIGPATH);
	writeToLogfile(toSend);
}
...
int clientApp()
{
    ...
    {
        sprintf(tmp, "WSAStart error %d", WSAGetLastError());
		writeToLogfile(tmp);
		tmp = '\0';
        return -1;
    }
    ...
    if(my_sock < 0)
    {
		sprintf(tmp, "Socket() error %d", WSAGetLastError());
		writeToLogfile(tmp);
		tmp = '\0';
        return -1;
    }
    ...
    if(inet_addr(SERVERADDR)!=INADDR_NONE)
        ...
        ((unsigned long*)&dest_addr.sin_addr)[0]=((unsigned long**)hst->h_addr_list)[0][0];
			...
			sprintf(tmp, "Invalid address %s", SERVERADDR);
			writeToLogfile(tmp);
			tmp = '\0';
            closesocket(my_sock);
            WSACleanup();
